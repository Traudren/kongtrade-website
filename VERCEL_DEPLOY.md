...
### Вариант B: Через Vercel Dashboard (не рекомендуется)

**Важно:** Миграции не должны выполняться автоматически при каждой сборке, так как:
- База данных может быть еще не настроена
- Миграции должны выполняться один раз, а не при каждой сборке
- Это замедляет процесс сборки

Используйте Вариант A (через CLI) для применения миграций.
