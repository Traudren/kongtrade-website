...
export async function POST(request: NextRequest) {
  try {
    ...
    const paidReferrals = user.referrals.filter((ref: any) => ref.subscriptions.length > 0).length
    ...
    // Record points usage (can create a separate table for this)
    // For now, just return successful result
