  ...
  const handleApprovePayment = async (paymentId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        alert(`Ошибка: ${error.message || 'Не удалось подтвердить платеж'}`)
      }
    } catch (error) {
      console.error('Payment approval error:', error)
      alert('Ошибка при подтверждении платежа. Попробуйте еще раз.')
    }
  }
  ...
  const handleRejectPayment = async (paymentId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json()
        alert(`Ошибка: ${error.message || 'Не удалось отклонить платеж'}`)
      }
    } catch (error) {
      console.error('Payment rejection error:', error)
      alert('Ошибка при отклонении платежа. Попробуйте еще раз.')
    }
  }
