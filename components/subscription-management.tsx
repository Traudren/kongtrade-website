...
export function SubscriptionManagement() {
  ...
  const handleApprovePayment = async (paymentId: string) => {
    try {
      ...
      if (response.ok) {
        alert('Платеж подтвержден и подписка активирована!')
        fetchPayments()
      } else {
        const error = await response.json()
        alert(`Error: ${error.message || 'Не удалось подтвердить платеж'}`)
      }
    } catch (error) {
      console.error('Payment approval error:', error)
      alert('Error при подтверждении платежа. Попробуйте еще раз.')
    }
  }
  ...
  const handleRejectPayment = async (paymentId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json()
        alert(`Error: ${error.message || 'Не удалось отклонить платеж'}`)
      }
    } catch (error) {
      console.error('Payment rejection error:', error)
      alert('Error при отклонении платежа. Попробуйте еще раз.')
    }
  }
    ...
    <Card className="glass-effect border-white/20">
      <CardHeader>
        ...
        <CardDescription className="text-gray-300">
          Управление userскими подписками и платежами
        </CardDescription>
